...
bl_pin = Pin(15, Pin.OUT)
bl = PWM(bl_pin)
bl.freq(1000)          # 1kHz frequency (typical for LEDs)
bl.duty_u16(65535)     # Max brightness (range 0-65535)

spi = SPI(0, baudrate=20000000, polarity=0, phase=0, sck=Pin(6), mosi=Pin(7),miso=None)
...
def test_main():
    tft.fill(TFT.BLACK)
    tft.text((0, 0), "     SANJANA <3 ADI     ", TFT.RED, sysfont, 1)
    time.sleep_ms(10000)
